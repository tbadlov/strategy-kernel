...
SCRIPT_CMD = f'python3 "{SKILL_DIR / "strategize.py"}"'
...
STRATEGIST_MINDSET = (
    ...
    "\n"
    "Think like a CEO, GM, or functional executive (CTO/CPO) applying\n"
    "Rumelt's framework. You are NOT:\n"
...
BAD_STRATEGY_SCAN_INSTRUCTIONS = (
    ...
    "```\n"
    "\n"
    "MANDATORY CHECKPOINT: Present the scan results to the user and ask\n"
    "(via AskUserQuestion) whether to refine the diagnosis or proceed.\n"
    "Do NOT advance without their explicit choice.\n"
    "If the diagnosis needs work, refine it together BEFORE proceeding.\n"
...
GUIDING_POLICY_INSTRUCTIONS = (
    ...
    "   - For INTERNAL strategies (technology platform, architecture, org\n"
    "     design, build-vs-buy): read 'competitor' as the status quo, vendor\n"
    "     alternatives, and organizational inertia — would the org actually\n"
    "     comply, or quietly route around this policy?\n"
    "\n"
...
SOURCES_OF_POWER_INSTRUCTIONS = (
    ...
    "\n"
    "For INTERNAL strategies (platform, architecture, org design), read\n"
    "'competitor' as the status quo, vendor alternatives, shadow IT, and\n"
    ...
    "```\n"
    "\n"
    "MANDATORY CHECKPOINT: Present the audit to the user and ask\n"
    "(via AskUserQuestion) which power opportunities to incorporate —\n"
    "or explicitly none — before proceeding. Do NOT advance without\n"
    "their explicit choice. If significant opportunities are chosen,\n"
    "revise the guiding policy or actions now.\n"
    "\n"
    "Then proceed to the next step."
)
...
CREATE_DESTROY_INSTRUCTIONS = (
    ...
    "   For INTERNAL strategies, the 'counter' is non-compliance and drift:\n"
    "   who quietly routes around this, and what does organizational inertia\n"
    "   do to it in 12 months?\n"
    "\n"
    "D. PRE-COMMIT NUMERIC THRESHOLDS:\n"
    "   Every gate, kill criterion, and evidence bar in the strategy must\n"
    "   state actual numbers ('>=20% pilot conversion at full price',\n"
    "   'NRR >= 108% by month 12'). 'Thresholds to be set later' is a\n"
    "   WEAK response — a board cannot hold anyone accountable to it.\n"
    "   The verdict cannot be ROBUST while any kill criterion lacks a number.\n"
    "\n"
    "E. VERDICT:\n"
    ...
    "\n"
    "   A first-pass ROBUST verdict must list at least 3 real weaknesses\n"
    "   found and explain why each does not require revision. A stress\n"
    "   test that finds nothing has not tested anything.\n"
    "\n"
    "OUTPUT FORMAT:\n"
    "```\n"
    ...
    "\n"
    "KILL CRITERIA (all numeric):\n"
    "  1. [metric + threshold + date]\n"
    "  2. [metric + threshold + date]\n"
    "\n"
    ...
    "  Diagnosis broken?  -> " + SCRIPT_CMD + " --step 2\n"
    "  Policy broken?     -> " + SCRIPT_CMD + " --step 4\n"
    "  Actions broken?    -> " + SCRIPT_CMD + " --step 5\n"
    "\n"
...
ARTIFACT_INSTRUCTIONS = (
    ...
    "\n"
    "The document should be written for an executive audience (CEO, board,\n"
    "exec team, or your organization's leadership) — clear,\n"
    "direct, and actionable. No fluff. No buzzwords. Every sentence should\n"
    "carry weight.\n"
    "\n"
    "DOCUMENT STRUCTURE:\n"
    "\n"
    "Write the strategy document in this format. Save it in the USER'S\n"
    "project directory (where the conversation is running) — NEVER inside\n"
    "the skill directory. Suggest the filename strategy-<topic-slug>.md\n"
    ...
    "[The financial spine. What this strategy costs and what it returns:\n"
    " revenue put at risk, expected trajectory (the honest shape, not the\n"
    " aspiration), burn and runway impact, and the arithmetic bridge\n"
    " between them. Numbers, not adjectives. The board will ask 'what\n"
    " does this cost us?' — this section answers it.]\n"
    "\n"
    "## Strategic Leverage\n"
    ...
    " Numeric kill criteria and review gates from the stress test.\n"
    " Early warning signs to monitor.]\n"
    "```\n"
    "\n"
    "WRITING GUIDELINES:\n"
    "- Write in plain, direct language. If Rumelt would call it fluff, remove it.\n"
    "- Keep framework machinery OUT of the document: no hallmark numbers,\n"
    "  no step names, no ROBUST/NEEDS REVISION verdicts, no Rumelt jargon.\n"
    "  The quality checks shaped the content; they are not content. If the\n"
    "  user wants the audit trail, offer it as a separate internal file\n"
    "  (e.g., strategy-quality-audit.md).\n"
